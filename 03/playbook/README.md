# Ansible Playbook: ClickHouse, Vector and Lighthouse Deployment

## Overview

Данный репозиторий содержит Ansible-playbook, предназначенный для установки и настройки ClickHouse, Vector и Lighthouse на разных группах хостов. Playbook поддерживает режимы `--check`, `--diff`, обработчики изменений, работу через шаблоны Jinja2 и идемпотентность.
...
В учебном окружении запуск осуществляется на реальных хостах через SSH подключение.
